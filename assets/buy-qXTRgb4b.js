import{P as l}from"./products-BIw6-FSk.js";const e=(t,n=document)=>n.querySelector(t),p=(t,n=document)=>[...n.querySelectorAll(t)],u=t=>"$"+Number(t).toLocaleString("en-US"),$="velox-bag",E=()=>{try{return JSON.parse(localStorage.getItem($)||"[]")}catch{return[]}},w=t=>{try{localStorage.setItem($,JSON.stringify(t))}catch{}},S=new URLSearchParams(location.search),A=S.get("id")||"lime",s={product:l.find(t=>t.id===A)||l[0],shot:0,size:"M",qty:1,cart:E(),front:"A"};function m(t){const n=(t.looks||[]).map((c,a)=>({src:c,label:"Look 0"+(a+1)})),o=(t.details||[]).map((c,a)=>({src:c,label:"Detail 0"+(a+1)}));return[...n,...o]}function q(t){const n=document.documentElement.style;n.setProperty("--accent",t.accent),n.setProperty("--accent-soft",t.accentSoft),n.setProperty("--glow",t.glow),document.documentElement.dataset.theme=t.id,document.title=`VELOX — ${t.name} ${t.colorway}`}function C(){const t=s.product,n=m(t);s.shot>=n.length&&(s.shot=0);const o=n[s.shot],c=s.front==="A"?e("#stageB"):e("#stageA"),a=s.front==="A"?e("#stageA"):e("#stageB");c.src=o.src,c.alt=`${t.name} ${o.label}`,c.classList.add("is-on"),a.classList.remove("is-on"),s.front=s.front==="A"?"B":"A",e("#shotBadge").textContent=o.label,p("#thumbs [data-shot]").forEach(i=>i.classList.toggle("is-on",+i.dataset.shot===s.shot))}function g(){const t=s.product,n=m(t);s.shot>=n.length&&(s.shot=0),q(t),e("#crumbs").innerHTML=`<a href="./">Home</a> / <a href="./#shop">Shop</a> / <span>${t.name} ${t.colorway}</span>`,e("#thumbs").innerHTML=n.map((a,i)=>`
  ...
  `).join("");const o=n[s.shot];e("#stageA").src=o.src,e("#stageA").alt=`${t.name} ${o.label}`,e("#stageA").classList.add("is-on"),e("#stageB").classList.remove("is-on"),s.front="A",e("#shotBadge").textContent=o.label,e("#fitNote").textContent=t.fit||"",e("#reviewCount").textContent=`${t.reviews||2} reviews`,e("#info").innerHTML=`
  ...
  `).join(""),z(),p("#thumbs [data-shot]").forEach(a=>a.addEventListener("click",()=>r(+a.dataset.shot)))}function r(t){const n=m(s.product);s.shot=(t+n.length)%n.length,C()}function z(){p("[data-size]").forEach(t=>t.addEventListener("click",()=>{s.size=t.dataset.size,g()})),p("[data-q]").forEach(t=>t.addEventListener("click",()=>{s.qty=Math.max(1,s.qty+Number(t.dataset.q)),g()})),e("#addBag")?.addEventListener("click",x)}function x(){const t=s.product,n=s.cart.find(o=>o.id===t.id&&o.size===s.size);n?n.qty+=s.qty:s.cart.push({id:t.id,qty:s.qty,size:s.size}),w(s.cart),_(),v(`${t.name} ${t.colorway} · ${s.size} added`),y()}function _(){const t=s.cart.reduce((i,d)=>i+d.qty,0),n=e("#cartCount");n.textContent=t,n.classList.toggle("is-on",t>0);const o=e("#cartItems"),c=e("#cartEmpty");s.cart.length?(c.classList.add("is-hidden"),o.innerHTML=s.cart.map(i=>{const d=l.find(h=>h.id===i.id);return`<div class="cart__item">
        <span class="cart__item-thumb" style="background-image:url(${d.looks?.[0]||d.background})"></span>
        <div class="cart__item-meta">
          <div class="cart__item-name">${d.name} ${d.colorway}</div>
          <div class="cart__item-gender">Size ${i.size}</div>
        </div>
        <div class="cart__item-price">${u(d.price*i.qty)}</div>
      </div>`}).join("")):(o.innerHTML="",c.classList.remove("is-hidden"));const a=s.cart.reduce((i,d)=>{const h=l.find(k=>k.id===d.id);return i+h.price*d.qty},0);e("#cartTotal").textContent=u(a)}function y(){e("#cart").classList.add("is-open"),e("#scrim").classList.add("is-open")}function b(){e("#cart").classList.remove("is-open"),e("#scrim").classList.remove("is-open")}function v(t){const n=e("#toast");n.textContent=t,n.classList.add("is-on"),clearTimeout(v._t),v._t=setTimeout(()=>n.classList.remove("is-on"),2200)}e("#prevShot").addEventListener("click",()=>r(s.shot-1));e("#nextShot").addEventListener("click",()=>r(s.shot+1));window.addEventListener("keydown",t=>{t.key==="ArrowRight"&&r(s.shot+1),t.key==="ArrowLeft"&&r(s.shot-1),t.key==="Escape"&&b()});let L=0;e("#stage").addEventListener("pointerdown",t=>{L=t.clientX});e("#stage").addEventListener("pointerup",t=>{const n=t.clientX-L;n>40&&r(s.shot-1),n<-40&&r(s.shot+1)});e("#navCart").addEventListener("click",y);e("#cartClose").addEventListener("click",b);e("#scrim").addEventListener("click",b);const f=e("#navBurger"),B=e("#navLinks");f?.addEventListener("click",()=>{const t=B.classList.toggle("is-open");f.setAttribute("aria-expanded",String(t))});const T=m(s.product)[0];e("#stageA").src=T.src;g();_();
